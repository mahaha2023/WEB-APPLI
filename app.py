...
st.markdown(
    """
    <div style="width: 100%; padding:10px; background-color: #FFFFFF; border: 2px solid black; text-align: center;">
        <h1 style="color: #000000; font-weight: bold; margin: 0;">Applications Web pour l'Analyse de Données et le Machine Learning</h1>
    </div>
    """, unsafe_allow_html=True)
...
if uploaded_file:
    ...
    if df is not None:
        ...
        # Afficher la somme des valeurs manquantes après le traitement
        st.subheader("Somme des données manquantes après traitement")
        missing_data_after = df.isnull().sum()
        st.write(missing_data_after[missing_data_after > 0])

        # Si aucune valeur manquante n'est présente
        if missing_data_after.sum() == 0:
          st.write("Toutes les données manquantes ont été traitées.")
